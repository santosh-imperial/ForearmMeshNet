...
class MeshEvaluationMetrics:
    ...
    class MeshEvaluationMetrics:
        ...
        def mesh_iou(self, pred_verts: torch.Tensor, target_verts: torch.Tensor,
            ...
            pitch = (bounds.max(0) - bounds.min(0)).max() / resolution if (bounds.ptp(0).max() > 0) else 1.0

            pv = pm.voxelized(pitch=pitch).matrix
            tv = tm.voxelized(pitch=pitch).matrix

            max_shape = np.maximum(pv.shape, tv.shape)
            pad = lambda a: np.pad(a, [(0, max_shape[0]-a.shape[0]),
                                    (0, max_shape[1]-a.shape[1]),
                                    (0, max_shape[2]-a.shape[2])], mode='constant')
            pv, tv = pad(pv), pad(tv)
        ...
        def compute_all_metrics(self,
            ...
            for name in pred_deformations.keys():
                ...
                orig_tpl = template_meshes[name]['vertices'].to(self.device)   # original template verts
                aff_tpl  = affine_verts_by_struct[name].to(self.device)        # affine template verts (from graph)
